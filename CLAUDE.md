...
**2026-07-22 cascade(TTA8-balanced+tau=0.15) LB 제출 — 기각 확정**: `scripts/run_test_cascade_probe.sh`
산출물을 실LB 제출한 결과 **0.92146**, 챔피언(0.93019) 대비 **−0.873pp(573그리드 기준 약 −5EM)로
뚜렷한 손실**. 단 인과 해석은 조심할 것 — overnight_log_2026-07-21.md 교차분석에 따르면 챔피언과
다른 19/819행 중 **실제 캐스케이드 발동(escalated)은 tau=0.15에서 겨우 4건뿐**이고 나머지 15건은
escalate 안 됐는데도 baseline과 갈린 것(=위 항목의 score24 비결정성 노이즈, 819건 중 15건/1.8%가
동일설정 재실행만으로도 갈리는 것과 같은 현상). 즉 관측된 −5EM 중 상당 부분은 캐스케이드 로직이
아니라 순수 재실행 노이즈일 가능성이 있어 "캐스케이드가 나쁘다"를 확정하긴 어렵다 — 그러나
실무적 결론은 어느 쪽이든 동일: **tau=0.15는 819건 중 7건에만 개입해 최선의 경우에도 상한이
~0.85pp뿐**(overnight_log 자체 추정)이라 손실이든 노이즈든 이 설정으로는 더 파고들 가치가 없음.
**cascade 축은 이 tau에서 종결** — 재도전하려면 LB 슬롯이 아니라 홀드아웃 재보정(τ·λ)부터
해야 함(`cascade.py` 자체 경고). TTA12(A4)와의 결합(`--tta 12 --cascade`)도 이 결과로 우선순위
하향 — cascade 성분 자체가 입증된 이득이 없는 채로 별 이유 없이 다른 base에 얹을 근거가 약함.
