...
**이 저장소 = Ver7을 베이스로 시작한 학습 단 개선 트랙 — 현재 대회 챔피언(LB 0.91623).
단 트랙 자체는 2026-07-20 종료**(챔피언 가중치·제출 구성은 그대로 유지, 예정됐던 hard-negative
본런은 미착수 — 마감까지 Ver11/Ver12에 자원 집중).
...
비유의(각 0.5979/0.6032/0.6032). 이어 **2026-07-20 서빙 뷰 세트를 TTA3→균형 TTA4(Klein 4원군,
`scripts/tta.py` BALANCED4)로 교체해 LB 0.91623 — 현 챔피언**(가중치 무변경, +0.524pp. 학습 없이
얻은 이득으로는 트랙 전체 최대). 상세 경위·게이트 판정 이력은 `../PROJECT_SUMMARY.md` §2 Ver8
