...
SFT 이후 포스트트레이닝 단계(노션 권장 순서: SFT → DPO → GRPO).
별도 보상모델 없이 라벨에서 선호쌍을 결정적으로 생성한다.
"""
...
def build_dpo_records(samples: list[Sample], cfg: DPOPairConfig) -> list[dict]:
    """TRL DPOTrainer 호환 레코드: {"prompt_messages", "chosen", "rejected", ...}.
    ...
    for s in samples:
        ...
        rejected_ranks = perm.adjacent_swap_ranks(aug.rank)
        rng.shuffle(rejected_ranks)
        rejected_ranks = rejected_ranks[: cfg.rejected_per_sample]
